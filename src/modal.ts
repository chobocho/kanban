...
export function openShell(
  dialogClass: string,
  onClosed: () => void,
): { dialog: HTMLDivElement; close(): void } {
  const overlay = document.createElement('div');
  overlay.className = 'modal-overlay';
...
export function openCardDetail(init: CardDetailInit, cb: CardDetailCallbacks): Promise<void> {
  return new Promise((resolve) => {
    ...
    const addCollapsibleSection = (
      ...
      bodyClass: string,
      hint: () => string,
    ): { body: HTMLDivElement; refresh(): void } => {
    ...
    const labelSection = addCollapsibleSection('labels', 'card-detail-labels', () =>
      assigned.size > 0 ? `(${assigned.size})` : '',
    );
    const labelList = labelSection.body;

    const buildAssignChip = (label: Label): HTMLElement => {
    ...
    const buildManageRow = (label: Label): HTMLElement => {
      const row = document.createElement('div');
    ...
    const dateBlock = (labelKey: string): HTMLDivElement => {
    ...
    const descSection = addCollapsibleSection('description', 'card-detail-desc-body', () =>
      desc.value.trim() ? '📝' : '',
    );
    ...
    const attachSection = addCollapsibleSection('attachments', 'card-detail-attachments', () =>
      init.attachments.length > 0 ? `(${init.attachments.length})` : '',
    );
    ...
    const commentSection = addCollapsibleSection('comments', 'card-detail-comments', () =>
      init.comments.length > 0 ? `(${init.comments.length})` : '',
    );
