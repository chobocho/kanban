...
| 기능 | 설명 |
|------|------|
| 📌 **보드 / 리스트 / 카드** | 추가 ➕ · 수정 ✏️ · 삭제 🗑️ (카드는 보관 🗄️ 후 복원 가능) |
| 🔍 **카드 상세 보기** | 카드를 열어 제목·설명·색상을 편집하고 삭제 (설명이 있으면 📝 배지 표시) |
| 🏷️ **라벨** | 보드별 색상 라벨을 카드에 지정·해제, 라벨 이름 변경 (카드 앞면에 색 칩 표시) |
| 🕒 **마감일** | 카드에 마감일시 지정·완료 표시 (지남🔴·임박🟡·완료🟢 색상 배지) |
| 🔎 **검색 / 필터** | 키워드 검색 + 라벨·마감 상태로 필터링 (리스트별 일치 카드 수 표시) |
| 🗄️ **보관함(아카이브)** | 카드를 보관 후 복원하거나 영구 삭제 (실수 삭제 방지) |
...
| 📂 **여러 보드 관리** | 🆕 새 보드 생성 · ✏️ 이름 변경 · 🗑️ 삭제 · 🔀 전환 |
...
```bash
npm test      # ✅ 단위 테스트 (순수 로직 25개)
npm run smoke # 🎭 브라우저 스모크 테스트 (Playwright)
```

- ✅ **단위 테스트**: 외부 프레임워크 없는 자체 하니스 (`test/`)
- 🎭 **스모크 테스트**: 렌더링 · 카드 추가 · 리스트 추가 · 실행 취소/다시 실행 · 언어 전환 · 줌 · 영속성 · 드래그 앤 드롭 검증
