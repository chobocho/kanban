 ...
 */
export function sortColumnCards(board, columnId, by) {
    const column = findColumn(board, columnId);
    if (!column)
        return false;
 ...
 */
export function duplicateColumn(board, columnId) {
    const index = board.columns.findIndex((c) => c.id === columnId);
    if (index < 0)
        return null;
...
export function moveAllCards(board, fromColumnId, toColumnId) {
 ...
 */
export function addCardsFromText(board, columnId, text) {
    const column = findColumn(board, columnId);
    if (!column)
        return [];
    const lines = text
...
export function addComment(board, columnId, cardId, text) {
    const trimmed = text.trim();
    const card = findCard(board, columnId, cardId);
    if (!card || !trimmed)
        return false;
...
export function updateComment(board, columnId, cardId, commentId, text) {
    const trimmed = text.trim();
    if (!trimmed)
        return false;
    const card = findCard(board, columnId, cardId);
...
export function removeComment(board, columnId, cardId, commentId) {
    const card = findCard(board, columnId, cardId);
    if (!card)
        return false;
 ...
 */
export function archiveCard(board, columnId, cardId) {
    const column = findColumn(board, columnId);
    if (!column)
        return false;
    const index = column.cards.findIndex((c) => c.id === cardId);
 ...
 */
export function restoreCard(board, cardId) {
...
export function deleteArchivedCard(board, cardId) {
    const index = board.archived.findIndex((a) => a.card.id === cardId);
    if (index < 0)
        return false;
