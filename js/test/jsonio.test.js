...
test('parseBoardJson reads a wrapped board export', () => {
    const board = parseBoardJson(JSON.stringify({ kanbanBoard: 1, board: { id: 'b1', name: 'X', columns: [] } }));
    assertEqual(board.name, 'X', 'board name kept');
});
test('parseBoardJson accepts a bare board object', () => {
    const board = parseBoardJson(JSON.stringify({ id: 'b1', name: 'Bare', columns: [{ title: 'C', cards: [] }] }));
