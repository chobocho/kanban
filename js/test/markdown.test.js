...
test('headings parse with levels 1-3', () => {
    ...
    assertEqual(blocks.map((b) => (b.kind === 'heading' ? b.level : 0)).join(','), '1,2,3', 'levels detected');
});
test('consecutive dash/star lines group into one unordered list', () => {
    ...
    if (blocks[0].kind === 'list')
        assertEqual(blocks[0].items.length, 3, 'three items');
});
...
test('links parse and unsafe schemes are rejected', () => {
    ...
    if (ok[0].kind === 'link')
        assertEqual(ok[0].href, 'https://example.com', 'href kept');
    const bad = parseInline('[x](javascript:alert(1))');
    assert(bad.every((s) => s.kind !== 'link'), 'javascript: link renders as plain text');
});
