...
import { buildMonthGrid, collectCalendarEvents, dayKey, groupEventsByDay, } from '../src/calendar.js';
...
test('collectCalendarEvents gathers start/due dates across all boards', () => {
    ...
    assertEqual(events.map((e) => `${e.cardId}:${e.kind}`).join(','), 'd:due,a:start,a:due', 'events are sorted by time ascending');
    const due = events[0];
